...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(
            Path(__file__).resolve().parent.parent, "logs", "pipeline_gipo.log"
        ), mode="a"),
    ]
)
...
def run_step4(args):
    ...
    logger.info(f"[Step 4] GIPO training with {args.model}...")
    logger.info(f"  SFT checkpoint: {sft_dir}")
    logger.info(f"  GIPO output:    {gipo_dir}")
    train_grpo(args.model, sft_dir, GRPO_DATA_PATH, gipo_dir, grpo_config)
...
def main():
    parser = argparse.ArgumentParser(description="AdaMacro GIPO Pipeline Runner")
    ...
    logger.info(f"Running GIPO pipeline steps: {steps} with model: {args.model}")
