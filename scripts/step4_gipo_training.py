...
def run_imagination_branch(
    ...
    result = env.execute(cf_tool_name, cf_arguments)
...
def train_grpo(
    ...
    logger.info(f"GIPO Training  G={G}  epochs={num_epochs}  lr={lr}  "
                f"grad_accum={grad_accum}  total_steps≈{total_steps}  "
                f"(Granularity-Imagination Policy Optimization)")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="AdaMacro Step 4: GIPO Training")
    ...
    logger.info("=" * 70)
    logger.info("AdaMacro Step 4: GIPO Training (Granularity-Imagination Policy Optimization)")
    logger.info("=" * 70)
