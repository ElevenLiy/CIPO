...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(
            Path(__file__).resolve().parent.parent, "logs", "pipeline_gipo_2gpu.log"
        ), mode="a"),
    ]
)
...
def _gipo_checkpoint_dir(model_name: str) -> str:
    ...
    return os.path.join(CHECKPOINT_DIR, f"gipo_2gpu_{short}")


def _gipo_eval_dir() -> str:
    return os.path.join(ADAMACRO_OUTPUT_DIR, "eval_gipo_2gpu_results")
...
def main():
    parser = argparse.ArgumentParser(description="AdaMacro GIPO 2-GPU Pipeline (7B/8B)")
    parser.add_argument("--model", type=str, default="qwen2.5-7b",
                       choices=["qwen2.5-7b", "llama3.1-8b", "qwen2.5-1.5b", "llama3.2-3b"])
    ...
    logger.info(f"Running GIPO-2GPU pipeline steps: {steps} with model: {args.model}")
