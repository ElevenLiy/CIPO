...
from configs.config import (
    ...
    ADAMACRO_OUTPUT_DIR,
    BPEConfig, SkillConfig, SFTConfig, GIPO7BConfig, EvalConfig,
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(
            Path(__file__).resolve().parent.parent, "logs", "pipeline_gipo_llama_2gpu.log"
        ), mode="a"),
    ]
)
...
def _gipo_checkpoint_dir(model_name: str) -> str:
    return os.path.join(CHECKPOINT_DIR, f"gipo_2gpu_{_model_short(model_name)}")


def _gipo_eval_dir(model_name: str) -> str:
    return os.path.join(ADAMACRO_OUTPUT_DIR, f"eval_gipo_2gpu_{_model_short(model_name)}_results")
...
def run_step4(args):
    ...
    logger.info(f"[Step 4] GIPO-2GPU training with {args.model}...")
...
def run_step5(args):
    ...
    logger.info(f"[Step 5] Evaluating {args.model} ({args.stage}) from GIPO-2GPU...")
...
def main():
    parser = argparse.ArgumentParser(description="AdaMacro GIPO 2-GPU Pipeline for LLaMA 3.1-8B")
    parser.add_argument("--model", type=str, default="llama3.1-8b",
                       choices=["llama3.1-8b", "llama3.2-3b", "qwen2.5-7b", "qwen2.5-1.5b"])
    ...
    logger.info(f"Running GIPO-LLaMA-2GPU pipeline steps: {steps} with model: {args.model}")
